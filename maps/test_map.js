...
var tiles_url = '../map_tiles/test_map/';  // this should eventually point to something local
var legend_max = 100;  // assume the minimum change is just negative max
var legend_unit = "units/mon";
...
function initMap() {
    ...
    // Set up the legend
    initLegend(legend_max, legend_unit);
    ...
    map.addListener('click', function(e) {
        ...
        var lat = center.lat();
        var lon = center.lng();
        var image_url = '../graphs/test_map/'+lon+',%20'+lat+'%20Data.jpg'   // use relative paths to avoid xss problems
...
function initLegend(max, unit) {
    // Create a legend with the given units and bounds.
    // Note that the colors are fairly approximate
    ...
    var slots = 25;
    html += "<div>";
    for (var i=slots; i>=-1; i--) {  // go to -1 to get a more solid red line at the bottom
        val = ((2*max)/(slots-1))*i-max;
        color = getColor(val,max);
        html += "<div style='text-align:center;width:100%;height:5px;background-color:"+color+";margin:1px;'></div>";
    }
    html += "</div>";

    // Create the rest of the html
    html += "<p style='text-align:center;'> -" + max + " " + unit + "</p>";
    div.innerHTML = html;
